...
pub async fn get_or_download_base_executable(target: BuildTarget) -> BuildResult<PathBuf> {
    ...
    let target_triple = format!("luneweb-{version}-{target}");

    let release_url = format!(
        "{base_url}/v{version}/{target_triple}.zip",
        base_url = "https://github.com/HighFlowey/lune-webview/releases/download",
    );
    ...
    let binary_file_name = format!("luneweb{}", target.exe_suffix());
